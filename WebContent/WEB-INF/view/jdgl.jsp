...
<script type="text/javascript">
	...
	function selectroles(){
		$("#rolesDg").datagrid({
			url: "${pageContext.request.contextPath}/selectAllRoles",
			...
			$.ajax({
				...
				success:function (res){
					if(res.success){
						$.ajax({
							type:"post",
							url:"${pageContext.request.contextPath}/addEditRoles",
							dataType:'json',
	...
	function deleteRoleInfo(index){
		...
		$.messager.confirm('确认','您确认想要删除记录吗？',function(r){
		    if (r){ // 用户点击了确认按钮
		        $.ajax({
					method:'post',
					url:'${pageContext.request.contextPath}/deleteRolesById',
					dataType:"json",
					data:{
						r_id:row.r_id
					},
	...
	function showRoles(index){
		...
		$("#treemenu").tree({
			url: '${pageContext.request.contextPath}/selectRolesModules',
			checkbox:true,
			lines: true,
			queryParams: {
				r_id:row.r_id
	...
	function baocun(){
		...
		$.ajax({
			url:'${pageContext.request.contextPath}/setupQuanXian',
